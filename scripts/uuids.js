...
function revString(str) {
    var splitString = str.split(""), reverseArray = splitString.reverse(), joinArray = reverseArray.join("");
    return joinArray;
}
...
// declare starting values
	var alpArr = alptoArr(alpStr, false), // get alphabet array without delimiter
...
// compress with delta
	...
	if ((pow > 1) && (!order)) { // we can't operate single symbol alphabet and not need to calculate delta if we keep order 
		...
		for (i = 0; i < arr.length; i += 1) { // loop by UUIDs
			...
			prev = next;
			if ((cutHexStr(item).length - 1) * 4 + strPow(cutHexStr(item).charAt(0)) < UNIT_LEN * 4 - pow) {item = cutHexStr(item); } // we use delimiter only if delta is less than UUID length minus one char
